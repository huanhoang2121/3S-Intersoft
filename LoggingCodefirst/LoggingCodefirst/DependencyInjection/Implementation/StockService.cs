...
namespace LoggingCodefirst.DependencyInjection.Implementation
{
    public class StockService : IStockService
    {
        ...
        public async Task<List<StockIndexViewModel>> GetListStockAsync()
        {
            var stocks = await _context.Stocks
                .Include(s => s.Store)
                .Include(s => s.Product)
                .ToListAsync();
            var viewModels = _mapper.Map<List<StockIndexViewModel>>(stocks);
            return viewModels;
        }

        public async Task<bool> CreateStockAsync(StockCreateViewModel stockCreateViewModel)
        {
            try
            {
        ...
        public async Task<StockEditViewModel> GetStockEditAsync(int? productid, int? storeid)
        {
            var stock = await _context.Stocks.FindAsync(productid, storeid);
            var viewModel = _mapper.Map<StockEditViewModel>(stock);
            return viewModel;
        }

        public async Task<bool> EditStockAsync(StockEditViewModel editViewModel)
        {
            try
            {
